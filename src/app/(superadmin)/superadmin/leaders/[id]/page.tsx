...
export default async function SuperAdminLeaderDetailPage({
  ...
  if (!leader) redirect('/superadmin/leaders')
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      {/* Header */}
      <div className="mb-6">
        <Link href="/superadmin/leaders" className="font-body text-xs text-white/30 hover:text-white/50 mb-2 block">
          ← Leaders
        </Link>
        <div className="flex items-center justify-between">
          ...
          <span className={`px-2.5 py-1 rounded-full text-xs font-body ${
            ...
            {leader.is_custom ? 'Org leader' : 'Global'}
          </span>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-white/3 rounded-xl p-1 mb-8 w-fit border border-white/6">
        {TABS.map(t => (
          <Link
            key={t.id}
            href={`/superadmin/leaders/${params.id}?tab=${t.id}`}
